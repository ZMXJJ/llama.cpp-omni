...
每个 Worker 占用一张 GPU，持有一个 UnifiedProcessor（PyTorch）或 CppBackendWorker（llama.cpp-omni），
提供 Chat (HTTP) / Streaming (WebSocket) / Duplex (WebSocket) 三种推理 API。

本文件位于 llama.cpp-omni 本地 app（`apps/server/`）。启动示例：

    cd llama.cpp-omni/apps/server
    # PyTorch：复制 config.example.json 为 config.json 并填写 model_path
    python worker.py --port 22700 --worker-index 0

    # C++ 后端：config.json 中 backend="cpp"，并配置 cpp_backend.llamacpp_root / model_dir
    python worker.py --port 22700 --worker-index 0
"""
...
class MiniCPMOWorker:
    ...
    def reset_half_duplex_session(
        ...
        if model is not None and hasattr(model, "reset_half_duplex_session"):
            try:
                ...
                logger.info(
                    f"[GPU {self.gpu_id}] Half-Duplex session reset via backend "
    ...
    def duplex_prepare(
        ...
        Args:
            ...
            system_content: 前端传入的 system_content 列表（优先），用于定制 C++ prompt
...
async def chat_ws(ws: WebSocket):
    ...
    try:
        ...
        try:
            ...
            # 从 system 消息的 content 里提取自定义 ref audio（若前端传了）
            _sys_content_items: List[dict] = []
            for m in raw_messages:
    ...
    try:
        while True:
            ...
            if msg_type == "prepare":
                ...
                _msn = msg.get("max_slice_nums")
                if _msn is None and config_dict:
                    _msn = config_dict.get("max_slice_nums")
                session_max_slice_nums = _msn if _msn is not None else 0
...
def main():
    ...
    if backend == "cpp":
        ...
        WORKER_CONFIG.update({
            ...
            "vision_backend": cpp_cfg.vision_backend,
        })
