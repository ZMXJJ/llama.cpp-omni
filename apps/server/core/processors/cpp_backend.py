...
def _build_prompts_from_content(
    ...
    输出结构：
      - voice_clone_prompt: "<|im_start|>system\\n{before}\\n<|audio_start|>"
      - assistant_prompt:   duplex    -> "<|audio_end|>{after}<|im_end|>\\n"
                            non-dup   -> "<|audio_end|>{after}<|im_end|>\\n<|im_start|>user\\n"

    其中 before = audio 前的所有 text 段拼接，after = audio 后的所有 text 段拼接。
    若无 audio 段，全部 text 归入 before。
    """
